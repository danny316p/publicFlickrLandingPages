// Flickr Sitemap Generator – FULL FINAL DROP-IN
// Node.js script
// Generates public.html and private.html with full UI
...
async function getCollections(){  
  const c = readCache("collections"); if(c) return c;
...
async function getUser(){
  const c = readCache("user"); if(c) return c;
  const d = await flickrCall("flickr.people.getInfo",{user_id:USER_ID});
  const p=d.person;
  const u={username:p.username._content, realname:p.realname._content, nsid:p.nsid, pathAlias:p.path_alias, iconfarm:p.iconfarm, iconserver:p.iconserver};
  writeCache("user", u); return u;
}

async function getTotalPhotos(){
  const c = readCache("total"); if(c) return c;
  const d = await flickrCall("flickr.people.getPhotos",{user_id:USER_ID,per_page:1});
  const t=parseInt(d.photos.total||0); writeCache("total", t); return t;
}
...
function thumb(ps) {
  if(ps.primary && ps.secret && ps.server && ps.farm)
    return `https://farm${ps.farm}.staticflickr.com/${ps.server}/${ps.primary}_${ps.secret}_q.jpg`;
  return null;
}
function avatarUrl(user){
  if(!user.iconserver || parseInt(user.iconserver)===0) return "https://www.flickr.com/images/buddyicon.gif";
  return `https://farm${user.iconfarm}.staticflickr.com/${user.iconserver}/buddyicons/${user.nsid}.jpg`;
}

// ---------- MAP + PUBLIC DETECTION ----------
function buildMap(list){
  const m={};
  list.forEach(ps=>{
    m[ps.id]={title:ps.title._content, photos:+ps.count_photos, videos:+ps.count_videos, primary:ps.primary, farm:ps.farm, server:ps.server, secret:ps.secret, isPublic:(+ps.count_photos+ +ps.count_videos) >0};
  });
  return m;
}
...
function enrich(col,map,mode){
  ...
  if(col.set){
    col.set = col.set.map(s=>{
      const m=map[s.id]; if(!m) return null;
      if(mode==="public" && !m.isPublic) return null;
      return {id:s.id, title:m.title, photos:m.photos, videos:m.videos, url:albumUrl(s.id), thumb:thumb(m)};
    }).filter(Boolean);
  }
  if(col.collection) col.collection = col.collection.map(c=>enrich(c,map,mode)).filter(Boolean);
  return col;
}
function prune(col){
  if(col.collection) col.collection = col.collection.map(prune).filter(Boolean);
...
function stats(col){
  ...
  if(col.set){ col.set.forEach(x=>{ s.albums++; s.photos+=x.photos||0; s.videos+=x.videos||0; }); }
  if(col.collection){ col.collection.forEach(c=>{ s.collections++; const sub=stats(c); s.collections+=sub.collections; s.albums+=sub.albums; s.photos+=sub.photos; s.videos+=sub.videos; }); }
  col._stats=s; return s;
}
function countCols(cols){ let n=0; (function walk(a){ a.forEach(c=>{ n++; if(c.collection) walk(c.collection); }); })(cols); return n; }
    ...
    return `<div class="collection">
      <div class="collection-header" onclick="toggle(this)">
        <span>
          ...
          <div class="meta">${col._stats.collections} collections • ${col._stats.albums} albums • ${col._stats.photos} photos${col._stats.videos?` • ${col._stats.videos} videos`:''}</div>
        </span>
        <span>[+]</span>
      </div>
      <div class="children">
        <div class="albums">${(col.set||[]).map(s=>`
          <a class="album-card" href="${s.url}" target="_blank" data-title="${s.title.toLowerCase()}" data-photos="${s.photos}" data-videos="${s.videos}">
            ...
            <div class="album-info"><div class="album-title">${s.title}</div><div class="meta">${s.photos} photos ${s.videos?`• ${s.videos} videos`:''}</div></div>
          </a>`).join('')}</div>
...
<div class="header">
  ...
  <div>
    ...
    <div class="meta">${totals.collections} collections • ${totals.albums} albums • ${totals.photos.toLocaleString()} photos</div>
  </div>
</div>
<div class="controls">
...
function expandAll(){document.querySelectorAll(".collection").forEach(c=>{c.classList.add("open");const i=c.querySelector(".collection-header span:last-child");if(i)i.textContent="[-]";});}
function collapseAll(){document.querySelectorAll(".collection").forEach(c=>{c.classList.remove("open");const i=c.querySelector(".collection-header span:last-child");if(i)i.textContent="[+]";});}
function setView(v){document.body.classList.remove("grid","list");document.body.classList.add(v);localStorage.setItem("view",v);const p=new URLSearchParams(window.location.search);p.set("view",v);window.history.replaceState({}, '', location.pathname+'?'+p);}
function filter(){
  const q=document.getElementById("search").value.toLowerCase();
  const min=+document.getElementById("minPhotos").value||0;
  const vid=document.getElementById("hasVideos").checked;
  const p=new URLSearchParams(window.location.search);p.set("q",q);p.set("minPhotos",min);vid?p.set("hasVideos","1"):p.delete("hasVideos");window.history.replaceState({}, '', location.pathname+'?'+p);
  document.querySelectorAll(".album-card").forEach(el=>el.classList.toggle("hidden",!(el.dataset.title.includes(q)&&+el.dataset.photos>=min&&(!vid||+el.dataset.videos>0))));
  document.querySelectorAll(".collection").forEach(col=>{const v=col.querySelectorAll(".album-card:not(.hidden)");col.classList.toggle("open",v.length>0);const i=col.querySelector(".collection-header span:last-child");if(i)i.textContent=col.classList.contains("open")?"[-]":"[+]";});
}
...
// ---------- MAIN ----------
(async()=>{
  const [collections, photosets, user, total] = await Promise.all([getCollections(), getPhotosets(), getUser(), getTotalPhotos()]);
  ...
  allTree.forEach(stats); publicTree.forEach(stats);
  const totals={albums:photosets.length, collections:countCols(allTree), photos:total};
  ...
  console.log("✅ DONE: private.html + public.html generated");
})();
